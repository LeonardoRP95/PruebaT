package com.prueba.pageObject;

import net.serenitybdd.core.pages.PageObject;
import org.openqa.selenium.By;

public class ElementsObject extends PageObject {

    By btnTextBox = By.xpath("//*[contains (text(), 'Text Box')]");
    By btnCheckBox = By.xpath("//*[contains (text(), 'Check Box')]");
    By btnButtons = By.xpath("//*[contains (text(), 'Buttons')]");
    By btnRadioButton = By.xpath("//*[contains (text(), 'Radio Button')]");
    By btnWebTables = By.xpath("//*[contains (text(), 'Web Tables')]");
    By btnUpload = By.xpath("//*[contains (text(), 'Upload and Download')]");
    By txtFullName = By.xpath("//*[@id='userName']");
    By txtEmail = By.xpath("//*[@id='userEmail']");
    By txtCurrentAddress = By.xpath("//*[@id='currentAddress']");
    By txtPermanentAddress = By.xpath("//*[@id='permanentAddress']");
    By btnSubmit = By.xpath("//*[@id='submit']");
    By valName =By.xpath("//*[@id='name']");
    By valEmail =By.xpath("//*[@id='email']");
    By valCurrentAddress =By.xpath("(//*[@id='currentAddress'])[2]");
    By valPermanentAddress =By.xpath("(//*[@id='permanentAddress'])[2]");
    By checkNotes = By.xpath("(//*[@class='rct-icon rct-icon-uncheck'])[3]");
    By checkWorkSpace = By.xpath("(//*[@class='rct-icon rct-icon-uncheck'])[3]");
    By checkOffice = By.xpath("(//*[@class='rct-icon rct-icon-uncheck'])[2]");
    By checkWordFile = By.xpath("(//*[@class='rct-icon rct-icon-uncheck'])[3]");
    By btnExpan = By.xpath("//*[@class='rct-option rct-option-expand-all']");
    By lstSelect = By.xpath("//*[@id='result']");
    By btnYesRadio = By.xpath("//*[@for='yesRadio']");
    By valRadio = By.xpath("//*[@class='text-success']");
    By btnAdd = By.xpath("//*[@id='addNewRecordButton']");
    By btnEdit = By.xpath("(//*[@title='Edit'])[last()]");
    By txtfirstName = By.xpath("//*[@id='firstName']");
    By txtlastName = By.xpath("//*[@id='lastName']");
    By txtemail = By.xpath("//*[@id='userEmail']");
    By txtage = By.xpath("//*[@id='age']");
    By txtsalary = By.xpath("//*[@id='salary']");
    By txtdepartament = By.xpath("//*[@id='department']");
    By extFirstName = By.xpath("(//*[@class='rt-tr -odd' or @class='rt-tr -even'])[last()]/div[1]");
    By extLastName = By.xpath("(//*[@class='rt-tr -odd' or @class='rt-tr -even'])[last()]/div[2]");
    By extEmail = By.xpath("(//*[@class='rt-tr -odd' or @class='rt-tr -even'])[last()]/div[4]");
    By extAge = By.xpath("(//*[@class='rt-tr -odd' or @class='rt-tr -even'])[last()]/div[3]");
    By extSalary = By.xpath("(//*[@class='rt-tr -odd' or @class='rt-tr -even'])[last()]/div[5]");
    By extDepartament = By.xpath("(//*[@class='rt-tr -odd' or @class='rt-tr -even'])[last()]/div[6]");
    By btnEliminar = By.xpath("(//*[@title='Delete'])[last()]");
    By btnDoubleClick = By.xpath("//*[@id='doubleClickBtn']");
    By btnRightClickMe = By.xpath("//*[@id='rightClickBtn']");
    By btnClickMe = By.xpath("(//*[@class='btn btn-primary'])[3]");
    By msnDoubleClick = By.xpath("//*[@id='doubleClickMessage']");
    By msnRightClickMe = By.xpath("//*[@id='rightClickMessage']");
    By msnClickMe = By.xpath("//*[@id='dynamicClickMessage']");
    By btnFile = By.xpath("//*[@type='file']");
    By btnDownload = By.xpath("//*[@href='data:image/jpeg;base64,']");

    By msjFile = By.xpath("//*[@id='uploadedFilePath']");
    public By getBtnDownload() {
        return btnDownload;
    }
    public By getMsjFile() {
        return msjFile;
    }
    public By getExtFirstName() {
        return extFirstName;
    }
    public By getMsnDoubleClick() {
        return msnDoubleClick;
    }
    public By getMsnRightClickMe() {
        return msnRightClickMe;
    }
    public By getMsnClickMe() {
        return msnClickMe;
    }
    public By getBtnDoubleClick() {
        return btnDoubleClick;
    }
    public By getBtnButtons() {
        return btnButtons;
    }
    public By getBtnRightClickMe() {
        return btnRightClickMe;
    }
    public By getBtnClickMe() {
        return btnClickMe;
    }
    public By getExtLastName() {
        return extLastName;
    }
    public By getExtEmail() {
        return extEmail;
    }
    public By getBtnEliminar() {
        return btnEliminar;
    }
    public By getExtAge() {
        return extAge;
    }
    public By getExtSalary() {
        return extSalary;
    }
    public By getExtDepartament() {
        return extDepartament;
    }
    public By getBtnWebTables() {
        return btnWebTables;
    }
    public By getBtnAdd() {
        return btnAdd;
    }
    public By getBtnEdit() {
        return btnEdit;
    }
    public By getTxtfirstName() {
        return txtfirstName;
    }
    public By getTxtlastName() {
        return txtlastName;
    }
    public By getTxtemail() {
        return txtemail;
    }
    public By getTxtage() {
        return txtage;
    }
    public By getTxtsalary() {
        return txtsalary;
    }
    public By getTxtdepartament() {
        return txtdepartament;
    }
    public By getBtnYesRadio() {
        return btnYesRadio;
    }
    public By getValRadio() {
        return valRadio;
    }
    public By getBtnRadioButton() {
        return btnRadioButton;
    }
    public By getBtnCheckBox() {
        return btnCheckBox;
    }
    public By getLstSelect() { return lstSelect;   }
    public By getBtnExpan() {return btnExpan;}
    public By getCheckNotes() {return checkNotes;}
    public By getCheckWorkSpace() { return checkWorkSpace;}
    public By getCheckOffice() {return checkOffice;}
    public By getCheckWordFile() {return checkWordFile;}
    public By getValName() { return valName; }
    public By getValEmail() { return valEmail; }
    public By getValCurrentAddress() { return valCurrentAddress; }
    public By getValPermanentAddress() { return valPermanentAddress; }
    public By getBtnTextBox() { return btnTextBox; }
    public By getTxtFullName() {
        return txtFullName;
    }
    public By getTxtEmail() {
        return txtEmail;
    }
    public By getTxtCurrentAddress() {
        return txtCurrentAddress;
    }
    public By getTxtPermanentAddress() {
        return txtPermanentAddress;
    }
    public By getBtnSubmit() {
        return btnSubmit;
    }
    public By getBtnUpload() {
        return btnUpload;
    }
    public By getBtnFile() {
        return btnFile;
    }

}
